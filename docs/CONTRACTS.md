...
**Symbols.** `IR_GOLD_FUND_AYAR` / `_TALA` / `_KAHRABA` (unit price, rial→toman, unit `unit`) and the composite `IR_GOLD_FUND_FLOW` (currency `PCT`): volume-weighted retail net flow `(ΣBuy_I_Volume − ΣSell_I_Volume)/Σtvol × 100` across configured funds — positive = individuals net-buying from institutions. `observed_at` derives from the API's Jalali date + Tehran time (converted; dedupes naturally after close). FLOW is exempt from jump/MAD suspect tests (`OSCILLATING_SYMBOLS`) — sign flips are normal, and neither corroboration path can rescue it: there is no second source, and the repetition path of Addendum 25 requires consecutive quotes to agree with each other, which a series that swings through zero never does. Sanity bounds ±100 still apply.
...
## Addendum 25 — a series that could not move, and a guard it could not leave (2026-08-13)

Three data-layer defects, one shared shape: a wrong assumption that nothing was positioned to contradict.

**US10Y was stored ten times too small for its entire history.** Yahoo publishes `^TNX` under two conventions — the legacy CBOE index (ten times the yield, `46.82` = 4.682%) and the plain yield (`4.697` = 4.697%) — and `core.normalize.tnx_to_pct` divided by ten unconditionally. Measured 2026-08-13: **1663 rows, 2021-07-26 to 2026-08-11, spanning 0.1174 to 0.4988, not one above 2** — real yields of 1.17% to 4.99% divided by ten, across both sources (`yahoo_backfill` 1254 rows, `yahoo` 409). On 2026-08-11 Yahoo switched to the index form, the correctly normalized 4.682 arrived, and against a last-good of 0.4697 it read as a **896% jump**: 81 observations held as suspect in 36 hours, none promoted, the series frozen.

The rule is now derived rather than assumed: `÷10` is a **repair**, applied only to a quote that cannot be a yield at all — above `SANITY_RANGES["US10Y"][1]` (25%, imported, not restated). At or below it the quote is taken at face value, because dividing a number that is already a valid yield is the failure being fixed. The residual ambiguity is stated, not hidden: 11.7 is 11.7% under one convention and 1.17% under the other and no single number settles it, so what catches a flip inside that band is the series (a tenfold step is suspect) rather than the quote. `raw_observations` now records which convention each row was read under (`unit` `TNX_index` vs `pct`), so the next switch is a `SELECT DISTINCT` away instead of a re-derivation.

**Migration 0022** repairs the rows: `prices` throughout, and in `raw_observations` only the backfill rows — those were written from the already-normalized value and carry the same error. The live rows keep their numbers, because `raw_value` is the record of what the provider said and rescaling it would falsify the audit trail that exists precisely so a normalization can be checked against its input; only their *label* is corrected. `value < 1.0` is the predicate: broken rows sit at or below 0.499, corrected ones start at 1.174, the band between is empty — so the update is idempotent and cannot reach a correctly normalized row. **No retrain is implied and none was run**: US10Y is in neither `FEATURE_SYMBOLS` (`IR_GOLD_18K`, `USD_IRT`, `XAUUSD`), nor `CONTEXT_SYMBOLS`, nor `FORECAST_SYMBOLS`; it is a charted series only, so the correction changes what the chart shows and nothing a model has ever seen.

**Deploy order: the code ships with or before migration 0022, never after.** The migration does not unfreeze the series by itself, and applied on its own it re-freezes it in the opposite direction — an earlier draft of this addendum and of the migration header claimed the reverse, on the assumption that Yahoo was still sending the index form. It is not. A live fetch on 2026-08-13 returned `regularMarketPrice` **4.627** (`chartPreviousClose` 4.682): the **plain-yield** convention. Replayed through `core.validation.classify_observation` against that live quote:

| code | migration 0022 | stored | verdict |
|---|---|---|---|
| old (unconditional `÷10`) | not applied | 0.4627 | `ok` — wrong scale, but flowing |
| old (unconditional `÷10`) | **applied** | 0.4627 | `suspect`, *jump of 90.1% vs last good value 4.697* — **frozen** |
| new (repair-only `÷10`) | not applied | 4.6270 | `suspect`, *jump of 885.1% vs last good value 0.4697* |
| new (repair-only `÷10`) | **applied** | 4.6270 | `ok` |

Only the two orders that put the code first are safe, and they are not equally safe — they are *both* safe for different reasons. Code-only is benign: the first quotes are held, but US10Y has one provider, no second source can ever arrive, and the repetition path accepts the new level after 5 observations spanning 90 minutes, so the series self-heals within about an hour and a half of the deploy while a WARNING sits in `app_issues`. Migration-only has no exit at all: every subsequent quote is another 90% jump against a level the old code cannot produce, so US10Y stays shut until the code lands. In the compose stack the api applies migrations at startup and `prediction-service` is recreated behind it, so `docker compose up -d --build` is already correctly ordered; the unsafe operation is `make migrate`, or redeploying the api alone, against an unchanged `prediction-service` image. The `.down.sql` mirrors the rule: roll the code back with or before the rollback.

**A single-source symbol could never escape `suspect`** — the deeper defect, and the one that turned a scale error into an outage. Suspect values were promoted only "when confirmed by a second source", and US10Y has exactly one provider. A guard a series cannot get out of is not a guard, it is a trapdoor: any genuine level shift in any single-source series froze it permanently, and silently. Corroboration now has a second form, along the time axis instead of across sources: a level the only available source has sustained across **`SUSTAIN_MIN_OBSERVATIONS` = 5** mutually consistent observations (same 3% `CONFIRM_TOLERANCE_PCT` — what counts as "the same level" should not depend on whether the evidence is spatial or temporal) spanning **`SUSTAIN_MIN_SPAN_MINUTES` = 90** is accepted.

Both conditions, never either. The count keeps a spike out — a one-off outlier is one observation and the next quote returns to the old level, so the run dies at one, and a source flapping between two levels never accumulates a run at all. The span keeps *cadence* out — five quotes inside a minute is one instant wearing five hats; at the 10-minute collect cron the floor is nine cycles. And the collect job adds the condition the thresholds cannot express: **the series must actually be frozen** (no accepted price from any source since the run began), so repetition is the fallback for the absence of corroboration and never a way for one provider to outvote a fallback that is still delivering. A frozen series also raises a WARNING at **`SUSPECT_ALERT_AFTER` = 3** — below the acceptance threshold on purpose, so the operator is told while the value is still being held rather than after it re-levels itself — mirrored into `app_issues` by `core.issues` and throttled to one per symbol per 3h (unthrottled, a stuck symbol would file ~144 rows a day and bury the incident under its own notifications). The original failure was not that the guard held a value; it was that holding it made no sound.

**TGJU disabled (migration 0023).** `call2`/`call3`/`call4.tgju.org` have answered scripted clients with "access denied" since 2026-08-02 10:55 — **2047 consecutive failures**, no success in between. The Addendum 19 breaker capped the cost at one attempt an hour, but an enabled provider that cannot succeed makes the roster lie: `/internal/providers/health` and the Issues tab both reported a failing source to chase rather than a source that is gone. Same treatment as Stooq in 0005, same reversibility — adapter, tests and history all kept, `enabled = FALSE` with the reason recorded, `consecutive_failures` reset so a manual re-enable gets a fair first attempt. Coverage was verified rather than assumed, against the registry rows and each adapter's symbol map: `IR_GOLD_18K` → hamrahgold (1), milligold (5); `USD_IRT` → bitmax (1); `XAUUSD` → yahoo (10), gold_api, metals_dev; `IR_COIN_EMAMI` → alanchand (20), brsapi (25), pricedb (30). Nothing loses its only source, and nothing changes in practice — a provider failing 2047 times in a row was already contributing no data. `app/seed/seed_history.py` constructs `TGJUProvider` directly rather than through the registry, so one-off history seeding is unaffected by the flag.

CI's migration job now seeds the three row shapes 0022 targets and asserts each branch. A data migration is invisible to a schema diff: without rows shaped like production's, it would be "tested" by never matching anything.

## Addendum 26 — three things the Addendum 25 work got wrong, and one it never asked (2026-08-13)

An adversarial audit of the Addendum 25 change set before it shipped. Every item below was reproduced by execution first, and the deploy-order correction is recorded in place under Addendum 25 rather than here, because it is a retraction of that addendum's own claim.

**The custom forecast could OOM the prediction service.** Addendum 25's sibling work made `/internal/predict/custom` spread its walk-forward folds over worker processes, sizing the pool with `min(joblib.cpu_count() - 1, 8)`. That reasons about cores and nothing else, and `docker-compose.yml` gives `prediction-service` a `memory: 1500m` limit and **no** `cpus` limit — so `joblib.cpu_count()` reports the *host's* cores and the pool sizes itself against hardware the container may never fill. Peak process-tree RSS for **one** request, measured by sampling the tree through a cold pool on the production shape (1224 daily points, 9 `FAST_CANDIDATES`, 40 folds):
...
Total RSS is linear in the worker count — ~260 MB of service plus ~184 MB per worker — and past ~5 workers the latency curve is flat, so the last three rows are pure memory bought for nothing. On an 8-core host the old rule chose 8 and a single forecast peaked **over the container limit**.

The count is now the smallest of three ceilings rather than one: cores − 1, `MAX_PARALLEL_FOLD_WORKERS` (8, where latency goes flat), and what the memory budget affords. The budget is *read*, not assumed — `training.memory_budget_bytes` takes the cgroup limit (v2 `memory.max`, v1 `memory.limit_in_bytes`; in a container `/sys/fs/cgroup` is the container's own root) and the machine's RAM and keeps the smaller, because a cgroup limit above physical memory is a formality. "Unlimited" needs no special-casing in either spelling: v2's `max` fails `int()` and v1's page-aligned 2^63−1 exceeds the hardware, and both fall through to RAM. `FOLD_POOL_MEMORY_SHARE = 0.5` of that budget goes to the pool, divided by a measured `FOLD_WORKER_RSS_BYTES = 200 MB`; the other half holds the service's own ~260 MB, whatever concurrent requests are carrying, and allocator slack. At `1500m` this resolves to **3 workers, measured peak 865 MB — 55% of the cap**, against 6.08–6.55s of latency where 8 workers bought 4.79s. Nothing is hardcoded for one deployment: a 400 MB container gets 1 worker, a 4 GB one gets 8. With no budget readable at all the answer is `FOLD_WORKERS_WITHOUT_A_BUDGET = 2` — an unknown budget must not mean an unbounded pool, and guessing high is the one mistake here with an unbounded cost.

**`parse_chart_history` read a `^TNX` history bar by bar.** Addendum 25 established that `÷10` is a repair applied only to a quote that cannot be a yield, and stated the residual ambiguity honestly: inside the band below the 25% ceiling, one number cannot settle its own convention. The history parser then applied that per-quote rule *per bar* — but a history payload is not a quote. It is one download under one convention, so the series is the evidence the single quote never had, and throwing it away loses exactly the ambiguous band. Fed a five-year index-form history:

| true | quote | per-bar | series-wide |
|---|---|---|---|
| 1.28% | 12.80 | **12.80** | 1.28 |
| 1.51% | 15.10 | **15.10** | 1.51 |
| 2.38% | 23.80 | **23.80** | 2.38 |
| 3.48% | 34.80 | 3.48 | 3.48 |
| 4.68% | 46.80 | 4.68 | 4.68 |

12.8, 15.1 and 23.8 all sit inside `SANITY_RANGES["US10Y"]` = (0.0, 25.0), so nothing downstream catches them either — a single backfill writes a series that is partly yields and partly index values, through `POST /internal/backfill/history`, the very job migration 0022 recommends for recovering the days the freeze cost. `core.normalize.tnx_series_to_pct` now asks the repair question once of the whole series: if **any** bar is above the ceiling the download is in the index convention and every bar is divided. Maximum, not median or majority — one impossible value is proof about the convention, whereas most values being possible is proof of nothing. What it still cannot settle is stated rather than hidden: a window whose true yield never exceeded 2.5% is below the ceiling under both readings. The ranges this is called with do not have that shape (a 3y or 5y range ending in 2026 contains the ~5% highs, i.e. an index near 50), but a short range over a low-rate era would.

**The escape hatch let the series out once and then re-trapped it.** Addendum 25's repetition path accepts a sustained new level from a single-source symbol. It did not tell the outlier test, and `RECENT_WINDOW` is 30 accepted values — so the next quote at the newly accepted level was still a MAD outlier against a window 29/30ths of which was the level the system had just agreed to abandon. The suspect run restarted at 1 and the full 5-observation / 90-minute wait was paid again, per price. Simulated end to end against `run_collect`, 400 cycles of the real 0.4697 → 4.682 shift at the 10-minute cron, one Yahoo quote per cycle:

| | first acceptance | subsequent acceptances | normal cadence resumes | prices in 400 cycles |
|---|---|---|---|---|
| before | cycle 10 (+100 min) | cycles 20, 30, 40 … 150 | cycle 150 (**+25.0 h**) | 265 |
| after | cycle 10 (+100 min) | every cycle from 11 | cycle 10 (**+1.7 h**) | 391 |
