...
Backfill the Iranian daily history — IR_GOLD_18K and USD_IRT, from TGJU daily
history then the pricedb GitHub dataset, with the bundled sample CSVs as a
last resort; idempotent:
...
This command no longer seeds XAUUSD. It used to fill it from whichever of TGJU
`ons` (spot), Stooq (spot) or Yahoo `GC=F` (COMEX futures) answered first —
three different instruments in one symbol, spliced at whatever date the source
changed. XAUUSD history comes from `POST /internal/backfill/history`
(`app/jobs/backfill.py`) instead: the same `GC=F` series the live collector
uses, an availability stamp of 23:00 UTC rather than this command's 12:00, and
a `raw_observations` audit row. `scripts/init.sh` calls both.

Deep Iranian history (back to 2010-2013, far beyond what live collection has
accumulated) is a separate job: `POST /internal/backfill/tgju-history`
(`app/jobs/tgju_backfill.py`). It writes only before the first existing
observation, refuses a symbol whose history is not internally coherent or does
not join onto the live series, and records the resulting splice in the
instrument note and in `app_settings['series_splices']`.
